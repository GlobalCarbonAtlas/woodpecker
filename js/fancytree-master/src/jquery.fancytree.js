...
//				alert("remove child markup for " + node);
	...
	nodeRenderTitle: function(ctx, title) {
		...
		var id, imageSrc, nodeTitle, role, tooltip,
			...
			icon = node.data.icon,
			// Ajout VMIPSL
			complexToolTip = node.data.complexToolTip;
...
//            nodeTitle = "<span role='treeitem' tabindex='-1' class='fancytree-title'" + id + tooltip + ">" + node.title + "</span>";
			...
			// Ajout VMIPSL
			if( complexToolTip )
               			 nodeTitle = "<span " + role + " class='fancytree-title'" + id + tooltip + ">" + node.title + "<div class='complexToolTip'>" + complexToolTip.replace( /\"/g, "&quot;" ) + "</div></span>";
		        else
