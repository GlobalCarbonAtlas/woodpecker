...
(function( $ )
{
    ...
    $.fn.contextmenu = function( option, menuId, menuTitleDiv, rebuild )
    {
        option = $.extend( { alias: menuId, width: 150 }, option );
        ...
        var buildItem = function( obj )
        {
            ...
            if( obj.disable )
            {
                ...
                if( "" != obj.icon )
                    T.className = "b-m-idisable subElement";
                else
        ...
        function showMenu( e, menutarget )
        {
            var groupId = menuId;
            target = menutarget;
            showMenuGroup.call( groups[groupId], { left: e.pageX, top: e.pageY }, 0 );
            $( document ).on( 'mousedown', hideMenuPane );
            $( "#" + groupId ).mouseleave( function()
            {
                $( this ).hide();
            } );
        }
